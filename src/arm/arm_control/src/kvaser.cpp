...
KvaserForElmo::KvaserForElmo(int channel_number, int TNumOfNodes, MOTOR* TpNode, const char* TNameOfNodes) :
    Kvaser(channel_number), NumOfNodes(TNumOfNodes), pNode(TpNode), NameOfNodes(TNameOfNodes)
{
    netInit();
}
/*析构*/
...
int KvaserForElmo::modeChoose(MOTOR*m, Mode mode)
{
    ...
    motorDisable(m);
    Tx.Byte[4] = mode;
    // 原代码等待 2s，6 个电机会导致总使能时间非常长；
    // 这里缩短为 200ms 左右，加快手臂整体验能速度。
    usleep(200000);
...
void KvaserForGold::GetPositionAndVelocity()
{
    ...
    for (int i = 0; i < NumOfNodes; i++)
    {
        ...
        for (int k = 0; k < Rx.DLC; k++)
            std::cout << std::hex << (int)Rx.Byte[k] << " ";
